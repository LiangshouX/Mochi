...
# 4. 从 TestPyPI 安装验证
pip install --index-url https://test.pypi.org/simple/ mochi-agent
...
publish-pypi job: 用 OIDC 认证 → 上传到 PyPI
       │
       ▼
用户可以 pip install mochi-agent==0.2.0 了 🎉
```
...
用户安装
─────────────────────────────────────────────────
  pip install mochi-agent        # 最新版本
  pip install mochi-agent==0.2.0 # 指定版本
```
...
- 如果用 Token：检查 token 是否正确、是否有 `mochi-agent` 项目的权限
